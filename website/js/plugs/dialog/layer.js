...
; !function (window, undefined) {
    ...
    var $, win, ready = {
        getPath: function () {
            ...
            for (var i = 0; i < js.length; i++) {
                var script = js[i];
                if (script.src.indexOf(jsn) === script.src.length - jsn.length) {
    ...
    Class.pt.creat = function () {
        ...
        that.vessel(conType, function (html, titleHTML) {
            ...
            if (config.type == 2) {
                try {
                    var fwin = $('#' + doms[4] + times)[0].contentWindow;
                    if (fwin) {
                        fwin._tool = config._tool;
                        fwin._dialog = config._tool || {};
                        fwin._dialog.close = function () { layer.close(that.index); }
                        fwin.params = config._tool.params;
                        fwin.closeLayer = function() { layer.close(that.index); };
                        fwin.minLayer = function() { layer.min(that.index); };
                        fwin.restoreLayer = function() { layer.restore(that.index); };
                        fwin.fullLayer = function() { layer.full(that.index); };
                        fwin.hideLayer = function() {
                            $('#' + doms[0] + that.index).hide(); 
                            $('#layui-layer-shade' + that.index).hide();
                        }
                        fwin.showLayer = function() {
                            $('#' + doms[0] + that.index).show();
                            $('#layui-layer-shade' + that.index).show();
                        }
                        fwin.iframeAuto = function () { layer.iframeAuto(that.index); }
                        fwin.iframeAuto2 = function () {
                            layer.iframeAuto(that.index);
                            that.offset();
                        }
                        if (window.publish)
                            publish("regeditWindow", fwin);
                    }
                } catch (e) {
        ...
        if (type === ready.type[1] && layero.attr('conType') === 'object') {
            ...
            if (type === ready.type[2]) {
                try {
                    ...
                    if (fwin.navigator.userAgent.indexOf("Chrome") > -1 || fwin.navigator.userAgent.indexOf("Firefox") > -1) //支持chrome,firefox
                    {
